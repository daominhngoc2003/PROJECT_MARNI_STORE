...
export const createBill = async (req, res) => {
  ...
  try {
    ...
    const info = await transporter.sendMail({
      from: `"6s Shoes 👟😘" ${process.env.EMAIL_SENDER}`,
      to: user.user_email,
      subject: "Thông tin đơn hàng từ 6s Shoes 👟",
      html: `
      <p style="font-size: 16px; color: #002140; font-weight: 600;">Cảm ơn bạn đã mua hàng từ 6s Shoes!</p>
      <p>Thông tin đơn hàng:</p>
      <ul>
        <li>Mã đơn hàng: ${bill_code}</li>
        <li>Tổng tiền: ${bill_totalOrder.toLocaleString("vi-VN", {
          style: "currency",
          currency: "VND",
        })}</li>
      </ul>
